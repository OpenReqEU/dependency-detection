...
public interface DependencyRepository extends CrudRepository<Dependency, String> {
    ...
    @Query("FROM Dependency WHERE (fromid = ?1 AND toid = ?2 OR fromid = ?2 AND toid = ?1) AND analysis_id = ?3")
    List<Dependency> findByIds(String fromid, String toid, Long analysisId);
